...
class TestTree(TestCase):
    ...
    def test_get_release(self):
        ...
        self.assertRaisesRegex(
            Exception,
            r"Unrecognised OFFICIAL setting: 'x'",
            Tree.get_release,
            self.config,
            "x",
        )
    ...
    def test_get_for_directory(self):
        ...
        for status, cls in (
            ...
            self.assertIsInstance(tree, cls)
            self.assertEqual(os.path.join(self.temp_dir, "www", "full"), tree.directory)
    ...
    def test_project_base(self):
        ...
        self.assertEqual(os.path.join(self.temp_dir, "kubuntu"), self.tree.project_base)
    ...
    def test_manifest_file_allowed_fails_bad_extensions(self):
        paths = [os.path.join(self.temp_dir, name) for name in ("foo.txt", "foo")]
        for path in paths:
            touch(path)
    ...
    def test_mark_current_trigger(
        ...
        with mkfile(current_triggers_path) as current_triggers:
            print("ubuntu\tdaily-live\tbionic-\tamd64", file=current_triggers)
        ...
        if pid == 0:  # child
            ...
            with open(
                os.path.join(publish_base, "20130321", ".marked_good"), "r"
            ) as marked_good:
                self.assertEqual("stonking-desktop-amd64.iso\n", marked_good.read())
...
class TestTags(TestCase):
    ...
    def test_span(self):
        ...
        self.assertEqual(
            '<span class="urgent">Sentence one.  Sentence two.</span>', str(tag)
        )

    def test_link(self):
        tag = Link("http://www.example.org/", "Example")
        self.assertEqual('<a href="http://www.example.org/">Example</a>', str(tag))
        tag = Link("http://www.example.org/", "Example", show_class=True)
        self.assertEqual(
            '<a class="http" href="http://www.example.org/">Example</a>', str(tag)
        )
...
class TestPublisher(TestCase):
    ...
    def test_publish_type(self):
        for image_type, project, dist, publish_type in (
            ("daily-preinstalled", "ubuntu-server", "bionic", "preinstalled-server"),
            ("daily-preinstalled", "ubuntu", "bionic", "preinstalled-desktop"),
            ...
            ("daily-live", "ubuntu-core-desktop", "mantic", "live-core-desktop"),
            ("daily-live", "ubuntu-core-installer", "noble", "ubuntu-core-installer"),
            ...
            if "_" not in image_type:
                self.assertEqual(image_type, Publisher._guess_image_type(publish_type))
...
class TestPublisherWebIndices(TestCase):
    ...
    def setUp(self):
        ...
        self.tree = Tree.get_for_directory(self.config, self.directory, "daily")
    ...
    def test_cssincludes(self):
        ...
        for project, expected in (
            ("ubuntu", [vanilla]),
            ("kubuntu", [vanilla, "//cdimage.ubuntu.com/include/kubuntu/style.css"]),
            ("lubuntu", [vanilla, "//cdimage.ubuntu.com/include/lubuntu/style.css"]),
            (
                "lubuntu-next",
    ...
    def test_cdtypestr(self):
        ...
        publisher = Publisher(self.tree, "daily-live")
        self.assertEqual("desktop image", publisher.cdtypestr("desktop", "iso"))
    ...
    def test_maybe_oversized(self):
        self.config["DIST"] = "bionic"
        oversized_path = os.path.join(self.directory, "bionic-desktop-i386.OVERSIZED")
        touch(oversized_path)
        publisher = Publisher(self.tree, "daily-live")
        desc = list(publisher.maybe_oversized("daily", oversized_path, "desktop"))
        self.assertEqual(
            "<br>\n"
    ...
    def test_mimetypestr(self):
        ...
        self.assertEqual("application/octet-stream", publisher.mimetypestr("img"))
    ...
    def test_web_heading(self):
        ...
        publisher = Publisher(self.tree, "daily")
        self.assertEqual(
            "Ubuntu 6.06.2 LTS (Dapper Drake)", publisher.web_heading("ubuntu-6.06.2")
        )
    ...
    def test_find_any_with_extension(self):
        for name in (
            "SHA256SUMS",
            "bionic-desktop-amd64.iso",
            "bionic-desktop-amd64.iso.torrent",
            "bionic-desktop-i386.iso",
            "bionic-desktop-i386.list",
        ):
            touch(os.path.join(self.directory, name))
        publisher = Publisher(self.tree, "daily-live")
        self.assertTrue(publisher.find_any_with_extension(self.directory, "iso"))
        self.assertTrue(
            publisher.find_any_with_extension(self.directory, "iso.torrent")
        )
        self.assertTrue(publisher.find_any_with_extension(self.directory, "list"))
        self.assertFalse(publisher.find_any_with_extension(self.directory, "manifest"))
    ...
    def test_make_web_indices(self):
        ...
        for name in (
            "SHA256SUMS",
            "bionic-desktop-amd64.iso",
            "bionic-desktop-amd64.iso.zsync",
            "bionic-desktop-i386.iso",
            "bionic-desktop-i386.list",
        ):
            touch(os.path.join(self.directory, name))
    ...
    def test_make_web_indices_for_simple_release(self):
        ...
        # not release
        directory = os.path.join(self.config.root, "www", "simple", "jammy")
        ...
        publisher.make_web_indices(directory, "ubuntu-22.04.2", status="release")
...
class TestDailyTree(TestCase):
    ...
    def test_default_directory(self):
        self.config.root = self.temp_dir
        self.assertEqual(
            os.path.join(self.temp_dir, "www", "full"), DailyTree(self.config).directory
        )

    def test_name_to_series(self):
        self.assertEqual("warty", self.tree.name_to_series("warty-install-i386.iso"))
...
class TestDailyTreePublisher(TestCase):
    ...
    def test_source_extension(self):
        self.assertEqual("raw", self.make_publisher("ubuntu", "daily").source_extension)

    def test_britney_report(self):
        self.assertEqual(
            os.path.join(self.config.root, "britney", "report", "kubuntu", "daily"),
    ...
    def test_publish_base(self):
        ...
        self.assertEqual(
            ...
            self.make_publisher("ubuntu-core-desktop", "daily-live").publish_base,
        )
        self.assertEqual(
            os.path.join(
        ...
        self.assertEqual(
            os.path.join(
                self.config.root, "www", "full", "kubuntu", "hoary", "daily-live"
            ),
            self.make_publisher("kubuntu", "daily-live").publish_base,
        )
    ...
    def test_size_limit(self):
        for project, dist, image_type, arch, size_limit in (
            ...
            self.assertEqual(
                size_limit, publisher.size_limit(arch), "%s/%s" % (project, dist)
            )

    def test_size_limit_extension(self):
        publisher = self.make_publisher("ubuntu", "daily")
        self.assertEqual(
            1024 * 1024 * 1024, publisher.size_limit_extension("armhf+omap4", "img")
        )
        self.assertEqual(
            1024 * 1024 * 1024, publisher.size_limit_extension("i386", "tar.gz")
        )
        self.assertEqual(
            publisher.size_limit("i386"), publisher.size_limit_extension("i386", "iso")
        )
    ...
    def test_new_publish_dir_honours_no_copy(self):
        ...
        touch(
            os.path.join(publish_current, "%s-alternate-i386.iso" % self.config.series)
        )
    ...
    def test_new_publish_dir_copies_same_series(self):
        ...
        publisher.new_publish_dir("20120807")
        self.assertEqual(
            [image], os.listdir(os.path.join(publisher.publish_base, "20120807"))
        )
    ...
    def test_new_publish_dir_prefers_pending(self):
        ...
        touch(
            os.path.join(publish_pending, "%s-alternate-amd64.iso" % self.config.series)
        )
    ...
    def test_publish_binary(self, mock_zsyncmake, *args):
        ...
        touch(os.path.join(source_dir, "%s-desktop-i386.raw" % self.config.series))
        touch(os.path.join(source_dir, "%s-desktop-i386.list" % self.config.series))
        touch(os.path.join(source_dir, "%s-desktop-i386.manifest" % self.config.series))
        ...
        mock_zsyncmake.assert_called_once_with(
            ...
            os.path.join(target_dir, "%s-desktop-i386.iso.zsync" % self.config.series),
            "%s-desktop-i386.iso" % self.config.series,
        )
    ...
    @mock.patch("cdimage.osextras.find_on_path", return_value=True)
    @mock.patch(
        "cdimage.tree.DailyTreePublisher.detect_image_extension", return_value="img.xz"
    )
    @mock.patch("cdimage.tree.zsyncmake")
    def test_publish_core_binary(self, mock_zsyncmake, *args):
        ...
        source_dir = publisher.image_output("amd64")
        touch(os.path.join(source_dir, "%s-live-core-amd64.raw" % self.config.series))
        touch(
            os.path.join(
                source_dir, "%s-live-core-amd64.model-assertion" % self.config.series
            )
        )
        self.capture_logging()
    ...
    def test_publish_appliance_binary(self, mock_zsyncmake, *args):
        ...
        touch(os.path.join(source_dir, "%s-live-core-amd64.qcow2" % self.config.series))
        self.capture_logging()
        list(publisher.publish_binary("live-core", "amd64", "20170429"))
        self.assertLogEqual(
            [
    ...
    def test_publish_canary_binary(self, mock_zsyncmake, *args):
        ...
        source_dir = publisher.image_output("amd64")
        touch(
            os.path.join(source_dir, "%s-desktop-canary-amd64.raw" % self.config.series)
        )
        touch(
            os.path.join(
                source_dir, "%s-desktop-canary-amd64.list" % self.config.series
            )
        )
        touch(
            os.path.join(
                source_dir, "%s-desktop-canary-amd64.manifest" % self.config.series
            )
        )
        self.capture_logging()
        ...
        mock_zsyncmake.assert_called_once_with(
            os.path.join(
                target_dir, "%s-desktop-canary-amd64.iso" % self.config.series
            ),
            os.path.join(
                target_dir, "%s-desktop-canary-amd64.iso.zsync" % self.config.series
            ),
            "%s-desktop-canary-amd64.iso" % self.config.series,
        )
    ...
    def test_publish_legacy_binary(self, mock_zsyncmake, *args):
        ...
        source_dir = publisher.image_output("amd64")
        touch(
            os.path.join(source_dir, "%s-desktop-legacy-amd64.raw" % self.config.series)
        )
        touch(
            os.path.join(
                source_dir, "%s-desktop-legacy-amd64.list" % self.config.series
            )
        )
        touch(
            os.path.join(
                source_dir, "%s-desktop-legacy-amd64.manifest" % self.config.series
            )
        )
        self.capture_logging()
        ...
        mock_zsyncmake.assert_called_once_with(
            os.path.join(
                target_dir, "%s-desktop-legacy-amd64.iso" % self.config.series
            ),
            os.path.join(
                target_dir, "%s-desktop-legacy-amd64.iso.zsync" % self.config.series
            ),
            "%s-desktop-legacy-amd64.iso" % self.config.series,
        )
    ...
    def test_publish_minimal_binary(self, mock_zsyncmake, *args):
        ...
        source_dir = publisher.image_output("amd64")
        touch(os.path.join(source_dir, "%s-minimal-amd64.raw" % self.config.series))
        touch(os.path.join(source_dir, "%s-minimal-amd64.list" % self.config.series))
        touch(
            os.path.join(source_dir, "%s-minimal-amd64.manifest" % self.config.series)
        )
        self.capture_logging()
        ...
        mock_zsyncmake.assert_called_once_with(
            os.path.join(target_dir, "%s-minimal-amd64.iso" % self.config.series),
            os.path.join(target_dir, "%s-minimal-amd64.iso.zsync" % self.config.series),
            "%s-minimal-amd64.iso" % self.config.series,
        )
    ...
    def test_publish_mini_iso_binary(self, mock_zsyncmake, *args):
        ...
        source_dir = publisher.image_output("amd64")
        touch(os.path.join(source_dir, "%s-mini-iso-amd64.raw" % self.config.series))
        self.capture_logging()
        ...
        mock_zsyncmake.assert_called_once_with(
            os.path.join(target_dir, "%s-mini-iso-amd64.iso" % self.config.series),
            os.path.join(
                target_dir, "%s-mini-iso-amd64.iso.zsync" % self.config.series
            ),
            "%s-mini-iso-amd64.iso" % self.config.series,
        )
        ...
        mock_zsyncmake.assert_has_calls(
            [
                mock.call(
                    ...
                    os.path.join(target_dir, "%s-src-1.iso.zsync" % self.config.series),
                    "%s-src-1.iso" % self.config.series,
                ),
                mock.call(
                    os.path.join(target_dir, "%s-src-2.iso" % self.config.series),
                    os.path.join(target_dir, "%s-src-2.iso.zsync" % self.config.series),
                    "%s-src-2.iso" % self.config.series,
                ),
            ]
        )
    ...
    def test_link(self):
        ...
        self.assertEqual(
            "20130319", os.readlink(os.path.join(publisher.publish_base, "current"))
        )
    ...
    def test_mark_current_single_to_mixed(self, mock_polish_directory):
        ...
        for date, arch in (("20130320", "i386"), ("20130321", "amd64")):
            for name in (
                ...
                path = os.path.join(publish_current, name)
                self.assertTrue(os.path.islink(path))
                self.assertEqual(os.path.join(os.pardir, date, name), os.readlink(path))
        self.assertCountEqual(
            [
    ...
    def test_mark_current_mixed_to_mixed(self, mock_polish_directory):
        ...
        for date in "20130320", "20130321":
            for name in (
                ...
                "bionic-desktop-amd64.iso",
                "bionic-desktop-amd64.manifest",
                "bionic-desktop-arm64.iso",
                "bionic-desktop-arm64.manifest",
                "bionic-desktop-i386.iso",
                "bionic-desktop-i386.manifest",
            ):
                touch(os.path.join(publisher.publish_base, date, name))
        ...
        self.assertEqual(
            [os.path.join(publisher.publish_base, "20130321")], publisher.checksum_dirs
        )
        mock_polish_directory.assert_called_once_with("current")
    ...
    def test_mark_current_ignores_old_series(self, mock_polish_directory):
        ...
        for name in (
            ...
            "saucy-desktop-amd64.manifest",
            "saucy-desktop-i386.iso",
            "saucy-desktop-i386.manifest",
        ):
            touch(os.path.join(target_dir, name))
        publish_current = os.path.join(publisher.publish_base, "current")
    ...
    def test_qa_product_main_tracker(self):
        for project, image_type, publish_type, product in (
            ...
            ("ubuntustudio", "daily-live", "desktop", "Ubuntu Studio Desktop"),
            ...
            ("ubuntu-budgie", "daily-live", "desktop", "Ubuntu Budgie Desktop"),
            ...
            ("ubuntu-wsl", "daily-live", "wsl", "Ubuntu WSL"),
        ):
            ...
            self.assertEqual(
                ("%s amd64" % product, "iso"),
                publisher.qa_product(project, image_type, publish_type, "amd64"),
            )
    ...
    def test_qa_product_ubuntu_preinstalled(self):
        for project, image_type, publish_type, arch, product in (
            ...
            self.assertEqual(
                (product, "iso"),
                publisher.qa_product(project, image_type, publish_type, arch),
            )
    ...
    def test_qa_product_lubuntu_preinstalled(self):
        ...
        self.assertEqual(
            ...
            publisher.qa_product(
                "lubuntu", "daily-preinstalled", "preinstalled-desktop", "armhf+ac100"
            ),
        )
    ...
    def test_cdimage_project_main_tracker(self):
        for project, image_type, publish_type, product in (
            ...
            self.assertEqual(
                ...
                publisher.cdimage_project("%s amd64" % product, "iso"),
            )
    ...
    def test_post_qa_oversized(self):
        ...
        publisher.post_qa("20130315", ["kubuntu/bionic/daily-live/bionic-desktop-i386"])
        expected_note = (
            "<strong>WARNING: This image is OVERSIZED. This should never "
            "happen during milestone testing.</strong>"
        )
    ...
    def test_polish_directory_no_metalink_focal(self, mock_make_web_indices, mock_call):
        self.config["DIST"] = "focal"
        publisher = self.make_publisher("ubuntu", "daily-live")
        target_dir = os.path.join(publisher.publish_base, "20130320")
        touch(os.path.join(target_dir, "%s-desktop-i386.iso" % self.config.series))
        self.capture_logging()
    ...
    def test_create_publish_info_file(self):
        ...
        target_dir = os.path.join(publisher.publish_base, "20130320")
        touch(os.path.join(target_dir, "%s-desktop-i386.iso" % self.config.series))
        touch(os.path.join(target_dir, "%s-desktop-i386.img" % self.config.series))
        touch(os.path.join(target_dir, "%s-desktop-i386.manifest" % self.config.series))
        self.capture_logging()
    ...
    def test_create_publish_info_file_current(self):
        ...
        osextras.symlink_force(
            os.path.join(source1_dir, iso1), os.path.join(target_dir, iso1)
        )
        osextras.symlink_force(
            os.path.join(source2_dir, iso2), os.path.join(target_dir, iso2)
        )
        self.capture_logging()
    ...
    def test_create_publish_info_file_current_is_link(self):
        ...
        touch(os.path.join(source_dir, "%s-desktop-i386.iso" % self.config.series))
        touch(os.path.join(source_dir, "%s-desktop-i386.img" % self.config.series))
    ...
    def test_generate_lxd_metadata(self, mock_generate):
        ...
        image_path = os.path.join(
            source_dir, "ubuntu-core-%s-amd64.img.xz" % self.config.core_series
        )
        touch(image_path)
    ...
    def test_publish_core(self, mock_post_qa, mock_metadata, *args):
        ...
        touch(
            os.path.join(source_dir, "%s-live-core-amd64.manifest" % self.config.series)
        )
        ...
        shutil.copy(
            xz_empty_source,
            os.path.join(source_dir, "%s-live-core-amd64.raw" % self.config.series),
        )
        ...
        mock_post_qa.assert_called_once_with(
            "20240718", ["ubuntu-core/24/edge/noble-live-core-amd64"]
        )
    ...
    def test_get_purge_data(self):
        ...
        self.assertEqual(2, publisher.get_purge_data("daily-live", "purge-days"))
    ...
    def test_get_purge_count(self):
        ...
        self.assertEqual(2, publisher.get_purge_data("daily-live", "purge-count"))
    ...
    def test_purge_removes_symlinks(self, *args):
        ...
        os.symlink("20130319", os.path.join(publisher.publish_base, "20130319.1"))
        with mkfile(os.path.join(self.temp_dir, "etc", "purge-days")) as purge_days:
            print("daily 1", file=purge_days)
        self.capture_logging()
        publisher.purge()
...
class TestReleasePublisherMixin:
    def test_daily_dir_normal(self):
        ...
        os.makedirs(path)
        self.assertEqual(path, publisher.daily_dir("daily", "20130327", "alternate"))
        self.config["PROJECT"] = "kubuntu"
        publisher = self.get_publisher()
        path = os.path.join(
    ...
    def test_daily_dir_preinstalled(self):
        ...
        publisher = self.get_publisher()
        path = os.path.join(
            self.temp_dir, "www", "full", "ubuntu", "daily-preinstalled", "20130327"
        )
        os.makedirs(path)
        self.assertEqual(
            path,
            publisher.daily_dir(
                "ubuntu", "ubuntu/daily-preinstalled/20130327", "daily-preinstalled"
            ),
        )
    ...
    def test_daily_dir_path_in_date(self):
        ...
        self.assertEqual(
            os.path.join(
                self.temp_dir, "www", "full", "ubuntu-server", "daily", "20130327"
            ),
        ...
        self.assertEqual(
            ...
            self.get_publisher().daily_dir(
                "daily-preinstalled",
                "ubuntu-server/daily-preinstalled/20130327",
                "server",
            ),
        )
    ...
    def test_daily_dir_source(self):
        self.config["PROJECT"] = "ubuntu"
        self.assertEqual(
            os.path.join(self.temp_dir, "www", "full", "daily", "20130327", "source"),
    ...
    def test_daily_base(self):
        ...
        self.assertEqual(
            os.path.join(
                self.temp_dir, "www", "full", "bionic", "daily", "20130327", "i386"
            ),
            self.get_publisher().daily_base("bionic/daily", "20130327", "wubi", "i386"),
        )
        self.config["DIST"] = "bionic"
        self.assertEqual(
            os.path.join(
    ...
    def test_remove_checksum(self):
        ...
        self.assertLogEqual(["checksum-remove --no-sign %s path" % self.temp_dir])
    ...
    def test_symlink(self):
        ...
        self.assertEqual(
            os.path.join(os.pardir, ".pool", "foo.iso"), os.readlink(dist_path)
        )
...
class TestFullReleasePublisher(TestCase, TestReleasePublisherMixin):
    ...
    def get_publisher(self, tree=None, image_type="daily", official="named", **kwargs):
        if tree is None:
    ...
    def test_target_dir(self):
        ...
        self.config["DIST"] = "bionic"
        self.assertEqual(
            os.path.join(self.temp_dir, "www", "full", "releases", "bionic", "release"),
    ...
    def test_version_link(self):
        ...
        self.config["PROJECT"] = "kubuntu"
        self.assertEqual(
            os.path.join(self.temp_dir, "www", "full", "kubuntu", "releases", "13.04"),
            self.get_publisher().version_link("daily"),
        )
    ...
    def test_torrent_dir(self):
        ...
        self.assertEqual(
            ...
            self.get_publisher(status="beta-2").torrent_dir("daily-live", "desktop"),
        )

    def test_want_torrent(self):
        self.assertTrue(self.get_publisher(official="named").want_torrent("desktop"))
        self.assertTrue(self.get_publisher(official="no").want_torrent("desktop"))
    ...
    def test_make_torrents(self, mock_check_call):
        self.config["CAPPROJECT"] = "Ubuntu"
        paths = [
            os.path.join(self.temp_dir, "dir", "ubuntu-13.04-desktop-%s.iso" % arch)
        ...
        self.capture_logging()
        publisher.make_torrents(os.path.join(self.temp_dir, "dir"), "ubuntu-13.04")
        self.assertLogEqual(["Creating torrent for %s ..." % path for path in paths])
        command_base = [
            "mktorrent",
            "-a",
            "https://torrent.ubuntu.com/announce",
            "--comment",
            "Ubuntu CD cdimage.ubuntu.com",
            "--output",
        ]
        mock_check_call.assert_has_calls(
            [
                mock.call(command_base + ["%s.torrent" % path, path], stdout=mock.ANY)
                for path in paths
            ]
        )
    ...
    def test_publish_release_arch_ubuntu_desktop_named(self, mock_call, *args):
        ...
        torrent_dir = os.path.join(
            self.temp_dir, "www", "torrent", "releases", "raring", "rc", "desktop"
        )
        osextras.ensuredir(target_dir)
        osextras.ensuredir(torrent_dir)
        self.capture_logging()
        publisher = self.get_publisher(official="named", status="rc")
        publisher.publish_release_arch("daily-live", "20130327", "desktop", "i386")
        self.assertLogEqual(
            [
        ...
        torrent_base = os.path.join(torrent_dir, "ubuntu-13.04-rc-desktop-i386")
        self.assertEqual(
    ...
    def test_publish_release_arch_ubuntu_desktop_no(self, mock_call, *args):
        ...
        torrent_dir = os.path.join(
            self.temp_dir, "www", "torrent", "releases", "bionic", "rc", "desktop"
        )
        osextras.ensuredir(target_dir)
        osextras.ensuredir(torrent_dir)
        self.capture_logging()
        ...
        self.assertCountEqual(
            [
                "bionic-desktop-i386.iso",
                "bionic-desktop-i386.iso.torrent",
                "bionic-desktop-i386.iso.zsync",
                "bionic-desktop-i386.manifest",
            ],
            os.listdir(target_dir),
        )
    ...
    def test_publish_release_kubuntu_desktop_named(self, mock_call, *args):
        ...
        target_dir = os.path.join(
            self.temp_dir, "www", "full", "kubuntu", "releases", series.name, "release"
        )
        torrent_dir = os.path.join(
            self.temp_dir,
            "www",
    ...
    def test_publish_release_test_no_purge_torrent(self, mock_call, *args):
        ...
        touch(
            os.path.join(torrent_dir, "kubuntu-%s-desktop-arm64.iso.torrent" % version)
        )
        touch(os.path.join(torrent_dir, "kubuntu-%s-desktop-arm64.iso" % version))
        self.capture_logging()
        publisher = self.get_publisher(official="named")
        publisher.publish_release("daily-live", "20130327", "desktop")
        self.assertCountEqual(
...
class TestSimpleReleasePublisher(TestCase, TestReleasePublisherMixin):
    ...
    def get_publisher(self, tree=None, image_type="daily", official="yes", **kwargs):
        if tree is None:
            tree = self.get_tree(official=official)
        return tree.get_publisher(image_type, official, **kwargs)
    ...
    def test_target_dir(self):
        ...
        self.config["PROJECT"] = "kubuntu"
        self.assertEqual(
            os.path.join(self.temp_dir, "www", "simple", "kubuntu", "bionic", "source"),
            self.get_publisher().target_dir("daily", "20130327", "src"),
        )
    ...
    def test_want_torrent(self):
        self.assertTrue(self.get_publisher(official="yes").want_torrent("desktop"))
        self.assertFalse(
    ...
    @mock.patch("cdimage.osextras.find_on_path", return_value=True)
    @mock.patch("subprocess.call", side_effect=call_mktorrent_zsyncmake)
    def test_publish_release_arch_ubuntu_desktop_poolonly(self, mock_call, *args):
        self.config["PROJECT"] = "ubuntu"
        self.config["CAPPROJECT"] = "Ubuntu"
        self.config["DIST"] = "raring"
        daily_dir = os.path.join(self.temp_dir, "www", "full", "daily-live", "20130327")
        ...
        self.assertFalse(
            os.path.exists(os.path.join(self.temp_dir, "www", "simple", "bionic"))
        )
        self.assertFalse(
            os.path.exists(
                os.path.join(
    ...
    def test_publish_release_kubuntu_desktop_yes(self, mock_call, *args):
        ...
        touch(os.path.join(daily_dir, "%s-desktop-i386.manifest" % series))
        touch(os.path.join(daily_dir, "%s-desktop-i386.iso.zsync" % series))
        pool_dir = os.path.join(self.temp_dir, "www", "simple", "kubuntu", ".pool")
        target_dir = os.path.join(
            self.temp_dir, "www", "simple", "kubuntu", series.name
        )
        torrent_dir = os.path.join(
            self.temp_dir, "www", "torrent", "kubuntu", "simple", series.name, "desktop"
        )
        self.capture_logging()
        ...
        self.assertFalse(
            os.path.exists(
                os.path.join(self.temp_dir, "www", "full", "kubuntu", "releases")
            )
        )
        self.assertTrue(
            os.path.exists(os.path.join(self.temp_dir, "www", "simple", ".manifest"))
        )
        self.assertTrue(
    ...
    def test_publish_release_simplestreams(self, mock_call, *args):
        ...
        self.assertFalse(
            os.path.exists(os.path.join(self.temp_dir, "www", "full", "releases"))
        )
